 ...
 *
 * SMTP and Zulip TLS, for a mail/Zulip server on an internal/self-signed CA
 * (the common case on an intranet with no public internet access), are
 * configured the same way as this app's OIDC and Rancher clients:
 *   SMTP_TLS_REJECT_UNAUTHORIZED  — "false" to skip verification entirely
 *                                    for SMTP specifically.
 *   ZULIP_CA_CERT_PATH          — PEM CA cert/bundle to trust for Zulip.
 *   ZULIP_TLS_REJECT_UNAUTHORIZED — "false" to skip verification entirely
 *                                    for Zulip specifically.
 * (SMTP has no CA-cert-path override of its own — NODE_EXTRA_CA_CERTS
 * already covers it, same as every other service.) Or set the global
 * TLS_REJECT_UNAUTHORIZED to skip verification for SMTP, Zulip, OIDC, and
 * Rancher all at once — see lib/tls.js. Internal/test environments only.
 */
...
async function sendEmail(settings, toEmail, subject, body) {
  const rejectUnauthorized = shouldRejectUnauthorized('SMTP_TLS_REJECT_UNAUTHORIZED');
  if (!rejectUnauthorized) {
    console.warn('[SMTP] WARNING: TLS certificate verification is DISABLED for SMTP (TLS_REJECT_UNAUTHORIZED or SMTP_TLS_REJECT_UNAUTHORIZED=false). Do not use this in production.');
  }
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: settings.smtp_user
      ...
      : undefined,
    // Only takes effect once TLS is actually in play (secure:true, or a
    // STARTTLS upgrade) — harmless to always pass. Unlike CA trust (which
    // NODE_EXTRA_CA_CERTS already gives nodemailer for free, same as any
    // other TLS client in this process), nodemailer has no implicit way to
    // pick up a "skip verification" flag, so this must be explicit.
...
function buildZulipAgent() {
  ...
  const rejectUnauthorized = shouldRejectUnauthorized('ZULIP_TLS_REJECT_UNAUTHORIZED');
  ...
  if (!rejectUnauthorized) {
    console.warn('[Zulip] WARNING: TLS certificate verification is DISABLED for the Zulip API (TLS_REJECT_UNAUTHORIZED or ZULIP_TLS_REJECT_UNAUTHORIZED=false). Do not use this in production.');
