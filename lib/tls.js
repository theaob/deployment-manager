 ...
 * app calls over HTTPS — Keycloak (OIDC), Rancher, Zulip, and SMTP. Each
 * has its own <SERVICE>_TLS_REJECT_UNAUTHORIZED env var (see
 * middleware/oidc-auth.js, lib/rancher.js, lib/notifications.js), but on
 * this app's typical intranet deployment every one of them commonly sits
 * on the same internal/self-signed CA — so the single global
 * TLS_REJECT_UNAUTHORIZED, set once, disables verification for all of them
 * at once instead of needing four identical flags.
 *
